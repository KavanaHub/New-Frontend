...
const STATUS_MAP = {
  ...
  submitted: { label: 'Disubmit', color: 'ctp-blue' },
};
...
export default function LaporanApprovePage() {
  ...
  const handleAction = async (mahasiswaId, status) => {
    try {
      const res = await dosenAPI.approveLaporan(mahasiswaId, status);
      if (res.ok) { toast.success(status === 'approved' ? 'Laporan disetujui!' : 'Laporan membutuhkan revisi'); setActionModal(null); loadData(); }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <Card className="bg-[hsl(var(--ctp-surface0)/0.55)] border-[hsl(var(--ctp-overlay0)/0.45)] ctp-ring"><CardContent className="pt-6"><p className="text-xs text-[hsl(var(--ctp-subtext0))]">Pending</p><p className="text-2xl font-bold text-[hsl(var(--ctp-yellow))]">{list.filter(l => l.status === 'pending').length}</p></CardContent></Card>
        <Card className="bg-[hsl(var(--ctp-surface0)/0.55)] border-[hsl(var(--ctp-overlay0)/0.45)] ctp-ring"><CardContent className="pt-6"><p className="text-xs text-[hsl(var(--ctp-subtext0))]">Approved</p><p className="text-2xl font-bold text-[hsl(var(--ctp-green))]">{list.filter(l => l.status === 'approved').length}</p></CardContent></Card>
      </div>

      <Card className="bg-[hsl(var(--ctp-surface0)/0.55)] border-[hsl(var(--ctp-overlay0)/0.45)] ctp-ring">
        <CardHeader className="flex flex-row items-start justify-between gap-3">
          <CardTitle className="flex items-center gap-2 text-[hsl(var(--ctp-text))]"><FileText className="h-4 w-4" /> Daftar Laporan</CardTitle>
        ...
        <CardContent>
          {filtered.length === 0 ? (
            <div className="text-center py-12"><FileText className="h-10 w-10 mx-auto text-[hsl(var(--ctp-overlay1))] mb-3" /><p className="text-sm text-[hsl(var(--ctp-subtext0))]">Tidak ada laporan</p></div>
          ) : (
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          <span className="text-sm font-semibold text-[hsl(var(--ctp-text))]">{l.mahasiswa_nama || '-'}</span>
                        ...
                        {l.file_url && <a href={l.file_url} target="_blank" rel="noreferrer" className="text-xs text-[hsl(var(--ctp-blue))] hover:underline inline-flex items-center gap-1 mt-1"><ExternalLink className="h-3 w-3" /> Buka Laporan</a>}
                      </div>
                      {l.status === 'pending' && (
                        <div className="flex gap-2 shrink-0">
                          <Button size="sm" onClick={() => handleAction(l.mahasiswa_id, 'approved')} className="rounded-xl h-8 bg-[hsl(var(--ctp-green)/0.20)] text-[hsl(var(--ctp-green))] border border-[hsl(var(--ctp-green)/0.35)]"><CheckCircle2 className="h-4 w-4" /></Button>
                          <Button size="sm" onClick={() => { setActionModal(l.mahasiswa_id); setCatatan(''); }} className="rounded-xl h-8 bg-[hsl(var(--ctp-red)/0.20)] text-[hsl(var(--ctp-red))] border border-[hsl(var(--ctp-red)/0.35)]"><XCircle className="h-4 w-4" /></Button>
                        </div>
                      )}
                    </div>
                  </div>
